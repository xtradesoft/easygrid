...
//            showRowNumber = false
    ...
    gridImplementations {
        ...
        jqgrid {
            ...
            formats = new HashMap<Class, Closure>([
                    (Date.class)    : { Date value -> new FormatTagLib().formatDate(date: value) },
                    (Long.class)    : { Long value -> value.toString() + "Juhu" },
                    (Calendar.class): { new FormatTagLib().formatDate(date: it) },
                    (Boolean.class) : { new FormatTagLib().formatBoolean(boolean: it) }
            ])
        }
