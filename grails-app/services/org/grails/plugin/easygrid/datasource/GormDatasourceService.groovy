...
class GormDatasourceService {
     ...
     * @param gridConfig
     */
    def generateDynamicColumns(GridConfig gridConfig) {

        // only generate if there are no columns defined
        if (!gridConfig.columns && gridConfig.domainClass) {
            PersistentEntity domainClass = resolveDomainClass(gridConfig.domainClass)

            def idProperty = domainClass.identity
            if (idProperty) {
            ...
            resolvePersistentProperties(domainClass).each { PersistentProperty prop ->
        ...
        (gridConfig.columns.elementList + gridConfig.filterForm?.fields?.elementList).findAll {
            ...
            if (property) {
                if (!filterable.filterDataType) {
                    final PersistentProperty columnProperty = getDomainProperty(gridConfig.domainClass, property)
...
//                }
     ...
     * @return
     */
    private List<PersistentProperty> resolvePersistentProperties(PersistentEntity domainClass, ignoreList = []) {
        final List<PersistentProperty> properties = domainClass.persistentProperties as List
...
//        Collections.sort(properties, new DomainClassPropertyComparator(domainClass))
        ...
        properties.sort { PersistentProperty o1, PersistentProperty o2 ->
            ...
            final Integer order1 = o1?.mapping?.mappedForm?.order
            final Integer order2 = o2?.mapping?.mappedForm?.order
