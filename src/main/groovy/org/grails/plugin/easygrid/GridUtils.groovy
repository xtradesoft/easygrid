...
class GridUtils {
    ...
    static valueOfPropertyColumn(GridConfig gridConfig, ColumnConfig column, element, idx) {
        ...
        log.trace "attempting to apply format to....${column}; ${val}"
        gridConfig.formats.each {
            log.trace "Format....${it}"
            log.trace "Format clazz....${it.key}"
            log.trace "Format closure....${it.value}"
            log.trace "Value class....${val.getClass()}"
        }
...
//            otherwise fallback to the slow implementation of Eval
    ...
    static PersistentEntity resolveDomainClass(Class beanClass) {
        GormEnhancer.findStaticApi(beanClass)?.gormPersistentEntity
    }

    static PersistentProperty getDomainProperty(Class clazz, String property) {
        resolveDomainClass(clazz)?.getPropertyByName(property)
    }
